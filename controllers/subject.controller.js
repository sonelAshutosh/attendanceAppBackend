...
exports.createSubject = asyncHandler(async (req, res, next) => {
  ...
  if (!name || !code) {
    return next(
      new AppError('Please provide a name and a code for the subject', 400)
    )
  }
  ...
  try {
    ...
    // If subject already exists (duplicate key error), return the existing subject
    if (error.code === 11000) {
      ...
      if (existingSubject) {
        return res.status(200).json({
          ...
          message: 'Subject already exists, using existing record.',
        })
      }
    }
...
exports.getSubjectById = asyncHandler(async (req, res, next) => {
  ...
  if (!subject) {
    return next(
      new AppError(`Subject not found with id of ${req.params.id}`, 404)
    )
  }
