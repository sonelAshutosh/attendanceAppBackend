...
exports.startSession = asyncHandler(async (req, res, next) => {
    const { classId, subjectId, attendanceType } = req.body;
    const teacherId = req.user._id;

    if (!classId || !subjectId || !attendanceType) {
        return next(new AppError('Please provide classId, subjectId, and attendanceType', 400));
    }

    // Ensure the teacher is assigned to this class
    const assignedClass = await Class.findOne({ _id: classId, teacherId: teacherId });
    if (!assignedClass) {
        return next(new AppError('You are not authorized to start a session for this class', 403));
    }
    
    // Check for an existing active session for the same class/subject
    const existingSession = await AttendanceSession.findOne({ classId, subjectId, status: 'Active' });
    if(existingSession) {
        return next(new AppError('An active session for this class and subject already exists', 400));
    }
...
exports.getActiveTeacherSessions = asyncHandler(async (req, res, next) => {
    const sessions = await AttendanceSession.find({ teacherId: req.user._id, status: 'Active' })
...
exports.markQrAttendance = asyncHandler(async (req, res, next) => {
    ...
    if (!classData.students.includes(studentProfile._id)) {
        return next(new AppError(`Student ${studentProfile.studentId} is not in this class.`, 400));
    }

    const existingRecord = await AttendanceRecord.findOne({ sessionId, studentProfileId: studentProfile._id });
    if (existingRecord) {
        return next(new AppError('Student has already been marked for this session', 400));
    }
...
exports.getStudentRecords = asyncHandler(async (req, res, next) => {
    ...
    const records = await AttendanceRecord.find({ studentProfileId: req.params.studentProfileId }).populate('sessionId');
    res.status(200).json({ success: true, count: records.length, data: records });
});
